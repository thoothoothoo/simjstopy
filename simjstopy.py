...
def aggregate_and_calculate_kpis(monthly_df):
    ...
    def calculate_kpis(agg_df):
        ...
        kpi['Operating Profit Margin (%)'] = ((agg_df['Revenue'] - agg_df['Total Costs']) / agg_df['Revenue'].replace(0, np.nan)) * 100 # Note: Profit = Revenue - Total Costs here
...
def create_summary_table(q_summary, y_summary, q_kpis, y_kpis, currency):
    ...
    fin_table = build_table(fin_summary_metrics, quarterly_summary, yearly_summary)
    kpi_table = build_table(kpi_metrics, quarterly_kpis, yearly_kpis)
...
if st.session_state.get('results_generated', False):
    ...
    monthly_table_formatted['Units Produced'] = monthly_table_formatted['Units Produced'].apply(format_units)
    # Hide unit econ columns for display
    monthly_table_formatted = monthly_table_formatted.drop(columns=['Var Cost/Unit', 'Sell Price/Unit'])
    monthly_table_formatted = monthly_table_formatted.set_index('Month')

    # Create charts based on current currency choice
    charts_data = create_charts(monthly_df, q_summary_raw, q_kpis_raw, selected_currency)

    # --- Display Section ---
    st.markdown("--- defenses ")
    st.header("📈 Projections Overview")

    tab1, tab2, tab3 = st.tabs(["Financial Summary", "Key Performance Indicators (KPIs)", "Monthly Details"])

    with tab1:
        st.subheader("Financial Summary Table")
        st.dataframe(fin_summary_table_formatted) # Use dataframe for better scrolling/sorting

        st.subheader(f"Quarterly Financial Summary Trends ({selected_currency})")
        if 'q_summary' in charts_data and not charts_data['q_summary'].empty:
            st.line_chart(charts_data['q_summary'])
        else:
             st.caption("Not enough data to display quarterly summary chart.")

    with tab2:
        st.subheader("KPI Table")
        st.dataframe(kpi_table_formatted)

        st.subheader("Quarterly KPI Trends (%)")
        if 'q_kpis' in charts_data and not charts_data['q_kpis'].empty:
             st.line_chart(charts_data['q_kpis'])
        else:
             st.caption("Not enough data or only non-percentage KPIs to display quarterly KPI chart.")

    with tab3:
        st.subheader("Monthly Details Table")
        st.dataframe(monthly_table_formatted)

        st.subheader(f"Monthly Trends ({selected_currency})")
        if 'monthly' in charts_data and not charts_data['monthly'].empty:
            st.line_chart(charts_data['monthly'])
        else:
            st.caption("No monthly data to display.")
else:
    st.info("Configure your inputs and click 'Generate Projections' to see the results.")
